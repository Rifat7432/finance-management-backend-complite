...
import { startOfMonth, endOfMonth, startOfYear, endOfYear, startOfDay, format, isValid, addDays, addMonths, addYears } from 'date-fns';
...
export const toUTC = (date: Date | string | number): Date => {
     ...
     const day = format(dateStr, 'dd');
     const monthName = format(dateStr, 'MMM');
     const year = format(dateStr, 'yyyy');
